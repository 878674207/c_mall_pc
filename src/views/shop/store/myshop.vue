          ...
          <el-tab-pane label="店铺信息" name="first">
            <div class="info-top flex-spb align-c">
              <div class="flex align-c">
                ...
                <span v-if="!isEdit"
                  ...
                  <i
                    ...
                    @click="isEdit = true"
                    style="color: #409eff; margin-left: 18px; cursor: pointer"
                /></span>
                <template v-else>
                  <el-input
                    size="mini"
              ...
              <div class="flex align-c">
                <span>店铺类型</span>
                <p class="infotyle">
                  {{ form.storeType === "0" ? "自营" : "入驻" }}
                </p>
              </div>
              <div class="flex align-c" style="margin-right: 100px">
                <span>店铺LOGO</span>
                <ImageUpload
                  :value="form.logo"
                  :limit="1"
                  ...
                  @input="selectMainImage($event, 'logo')"
                />
              </div>
            </div>
            ...
            <div class="bgitem">
              <div class="itembottom flex align-c">
                <span>主营范围</span>
                <span v-if="!isEdit"
                  >{{ form.mainBusiness || "--" }}
                  <i
            ...
            <el-form-item label="相关资质证明" prop="attest">
              <el-image
                style="
                  ...
                  border-radius: 10px;
                "
                v-for="(item, index) in form.attest"
                :key="index"
                :src="item.filePath"
                :preview-src-list="getfilePath()"
              >
              </el-image>
            </el-form-item>
          </el-tab-pane>
...
<script>
import { bcurrentStoreInfo, bupdateStore } from "@/api/shop/index.js";
...
export default {
  ...
  data() {
    return { activeName: "first", form: {}, rules: {}, isEdit: false };
  },
  computed: {},
  created() {
    this.getUser();
  ...
  methods: {
    ...
    submitForm() {
      bupdateStore(this.form).then((res) => {
        this.$message.success("修改成功");
        this.isEdit = false;
      });
    },
    selectMainImage(e, lable) {
      this.form[lable] = e.length > 0 ? e[0].fileUrl : null;
      bupdateStore(this.form);
    },
    changeType() {},
    handleClick(tab) {
    ...
    getUser() {
      bcurrentStoreInfo().then((response) => {
